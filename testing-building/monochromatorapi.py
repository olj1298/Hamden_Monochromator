...
def scanbasic(MCport,wlbegin,wllast,t):
    """
    Moves controller from home to a desired wavelength, pauses for an exposure time, then moves to a final wavelength. Assumes that controller begins at home.
        Inputs:
            :MCport(string): Serial Port connection
            :wlbeginfloat): Desired wavelength
            :wllast(float): Desired final wavelength
    ...
    try:
        ...
        rev = 9000.0 #steps
        #equation to find difference between home position and new wavelength desired
        diffstart = wlbegin - home_wl
        diffend = wllast - wlbegin
        ...
        if lowlim < wlbegin <  uplim:
            ...
            while mvread > 0: #some type of motion
                ...
                if mvread == 0: #no motion
                    ...
                    print(msg)
        print(f"pausing for {t} second exposure")
        wait(MCport,t)
        print("continuing to second wavelength")
        if lowlim < wllast <  uplim:
            ser = serial.Serial(port=MCport, 
                                baudrate = 9600, 
                                timeout = None,
                                xonxoff = True,
...
def scancsv(MCport,filename):
    """
    Reads user's csv file for desired wavelengths and exposure times. Assumes controller begins at home wavelength.
        Inputs:
            :MCport(string): Serial Port connection
            :filename(string)): File Location of user's csv file
        Returns:
            ::Movement status, exposure status, movement completion
            ::Error message if exception occurs
    """
