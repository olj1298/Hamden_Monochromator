...
def pixis_load_experiment(filename='DemoExp'):
    ...
    connected=experiment.Load(filename)
    if connected == False: 
...
def slitposition(MC_lamp,Traget_spectral_bandpass):
    ...
    MC_lamp= 'Xe' #Which lamp is on  Xe or D2? #make variable user can change
    Traget_spectral_bandpass= 0.62 #nm #make variable user can change
...
def experimentfilesettings(exp_filenames_basename,exp_folder_name):
    """
    ...
    """
    exp_filenames_basename='exp_Xe_500ms' #make variable user can change
    exp_folder_name=exp_filenames_basename+'_180to700nm_10C' #make variable user can change
...
def experimentrun(start_wl,end_wl,wl_step,exp_time,n_bias):
    """
    ...
    """
    start_wl=180.0 #nm #make variable user can change
    end_wl=700.0 #nm #make variable user can change
    wl_step=10.0 #nm #make variable user can change
    exp_time=1000.0 #milleseconds #make variable user can change
    ndark=int(1) #int
